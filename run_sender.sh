...
# --start-bitrate-kbps auto (derived from the resolution, logged), 60 s. Traces -> results/<ts>-sender/app
# (tx-*.csv, tx-stats.jsonl, sender.log). Copy that app/ next to the receiver's for `make verify`.
#
...
RD="results/$(date +%Y%m%d-%H%M%S)-sender"
mkdir -p "$RD/app"
...
"$BIN" --signaling-host "$HOST" --signaling-port 8765 --trace-dir "$RD/app" \
       ...
       --degradation maintain_resolution --start-bitrate-kbps auto --duration 60 "$@" 2>&1 | (trap '' INT; exec tee "$RD/app/sender.log")
